...
def radiusfinder(COM, array_4D, filename, NOF, a, size, outfile, moleculefile, indexing, margin):
    array = array_4D
    s1 = int(a / size)
    ...
    for k in range(NOF):
        ...
        for l in range(s):
            ...
            elif raw_radius[k][l] > radius_is[k] and raw_radius[k][l] < margin * mean:
                status[k][l] = "outside"
                upcount[k] += 1
                upradius[k] += raw_radius[k][l]
            elif raw_radius[k][l] >= margin * mean:
                status[k][l] = "expelled"
                expelled[k] += 1
        loradius[k] /= locount[k]
        upradius[k] /= upcount[k]
    return radius_is, mean, upradius, upcount, loradius, locount, expelled, status, 
