def read_file_rem(filename, NOF, a, size, newfile):
    y = 0
    f = open(filename, "r")
    lines = f.readlines()
...
def create_array_3D_rem(array_2D, filename, NOF, a , size, newfile):
    ...
    for k in range(pages):
        for i in range(rows):
            array3d[i][0][k] = array[h][0]
            array3d[i][1][k] = array[h][1]
            array3d[i][2][k] = array[h][2]
            array3d[i][3][k] = array[h][3]
            h = h + 1
...
def create_array_4D_rem(array_3D, filename, NOF, a, size, newfile):
