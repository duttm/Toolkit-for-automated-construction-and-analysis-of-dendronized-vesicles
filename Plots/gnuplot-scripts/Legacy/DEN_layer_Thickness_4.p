...
filename = "Den_layer_thickness"

yaxis = "Thickness (nm)"

set output filename.'_acidic.png'
...
f(x) = m * x + q     
fit f(x) '../DATA/ACIDIC/'.filename.'/SummaryG1.txt' using  ($2):($3) via m, q
fit f(x) '../DATA/ACIDIC/'.filename.'/SummaryG2.txt' using  ($2):($3) via m, q
fit f(x) '../DATA/ACIDIC/'.filename.'/SummaryG3.txt' using  ($2):($3) via m, q
fit f(x) '../DATA/ACIDIC/'.filename.'/SummaryG4.txt' using  ($2):($3) via m, q
fit f(x) '../DATA/ACIDIC/'.filename.'/SummaryG5.txt' using  ($2):($3) via m, q
fit f(x) '../DATA/ACIDIC/'.filename.'/SummaryG6.txt' using  ($2):($3) via m, q
...
plot '../DATA/ACIDIC/'.filename.'/SummaryG1.txt'  using  ($2):($3):($4) with yerrorbars ls 7 ps 0.1 lt rgb "black" linewidth 3 not,\
 '../DATA/ACIDIC/'.filename.'/SummaryG1.txt'  using  ($2):($3) with linespoints linestyle 1 title "G1-Acidic" ,\
  '../DATA/ACIDIC/'.filename.'/SummaryG2.txt'  using  ($2):($3):($4) with yerrorbars ls 7 ps 0.1 lt rgb "green" linewidth 3 not,\
